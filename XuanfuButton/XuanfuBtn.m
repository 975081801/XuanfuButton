...
@interface XuanfuBtn()<UIDynamicAnimatorDelegate>
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _imageView = [[UIImageView alloc]initWithImage:[[UIImage imageNamed:@"1"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate]];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (( self.endPoint.x - self.startPoint.x >= -errorRange)||self.endPoint.y - self.startPoint.y >= -errorRange||self.startPoint.x - self.endPoint.x >= -errorRange||self.startPoint.y - self.endPoint.y >= -errorRange){
        ...
        // 获取屏幕边框位置
        CGFloat topRange = self.endPoint.y;
        CGFloat bottomRange = superheight - topRange;
        CGFloat leftRange = self.endPoint.x;
        CGFloat rightRange = superwidth - leftRange;
         ...
         if (minRange == topRange) {// 上边距最小
             
             //上
             
             endX = endX - kOffSet > superwidth ? superwidth - kOffSet : endX;
             
             minPoint = CGPointMake(endX ,kOffSet);
             ...
             minPoint = CGPointMake(kOffSet  , endY);
